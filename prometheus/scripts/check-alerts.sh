...
echo "${GREEN}╔═══════════════════════════════════════╗${NC}"
echo "${GREEN}║   Prometheus Alerts & Rules           ║${NC}"
echo "${GREEN}╚═══════════════════════════════════════╝${NC}"
echo ""
...
if [[ "$FIRING" -gt 0 ]]; then
    echo "  ${RED}🔥 $FIRING firing alert(s)${NC}"
    echo ""
    echo "$ALERTS" | jq -r '
    ...
    echo "  ${GREEN}✓ No firing alerts${NC}"
fi

if [[ "$PENDING" -gt 0 ]]; then
    printf "\n"
echo "  ${YELLOW}⏳ $PENDING pending alert(s)${NC}"
...
printf "  Total Groups:    $TOTAL_GROUPS\n"
printf "  Alerting Rules:  $TOTAL_ALERTING\n"
printf "  Recording Rules: $TOTAL_RECORDING\n"

#############################################
# Rules by Group
#############################################
printf "\n"
echo "${BLUE}═══ Rules by Group ═══${NC}"
...
#############################################
printf "\n"
echo "${BLUE}═══ Alert Rules by Severity ═══${NC}"
echo "$RULES" | jq -r '
...
' 2>/dev/null | while read line; do
    ...
    case "$severity" in
        critical) echo "  ${RED}$severity: $count${NC}" ;;
        warning)  echo "  ${YELLOW}$severity: $count${NC}" ;;
        info)     echo "  ${CYAN}$severity: $count${NC}" ;;
...
if [[ "$UNHEALTHY" -gt 0 && "$UNHEALTHY" != "null" ]]; then
    echo "  ${RED}⚠ $UNHEALTHY unhealthy rule(s)${NC}"
    echo "$RULES" | jq -r '
      .data.groups[].rules[] | select(.health != "ok" and .health != null) |
      "    - \(.name): \(.health) - \(.lastError // "no error")"
    ' 2>/dev/null | head -10
else
    echo "  ${GREEN}✓ All rules healthy${NC}"
fi

printf "\n"
echo "${GREEN}✓ Alert analysis complete${NC}"
