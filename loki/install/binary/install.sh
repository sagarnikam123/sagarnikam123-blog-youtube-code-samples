...
create_directories() {
    ...
    mkdir -p "$STACK_DIR"/{loki,grafana,prometheus,minio,archive} || error_exit "Failed to create stack directories"
...
setup_grafana() {
    ...
    extracted_dir=$(find . -maxdepth 1 -type d -name "grafana*" ! -name "grafana" | head -1)
    if [[ -z "$extracted_dir" ]]; then
        error_exit "Could not find extracted Grafana directory"
    fi

    # Remove existing grafana directory and rename extracted one
    rm -rf grafana 2>/dev/null || true
    if ! mv "$extracted_dir" grafana; then
        error_exit "Failed to rename Grafana directory"
    fi
...
main() {
    ...
    echo ""
    echo "Data storage:"
    echo "  $DATA_DIR/              - Runtime data"
    echo ""
    echo "Configs & scripts are in the git repository:"
    echo "  loki/configs/           - Loki configuration files"
    echo "  loki/install/binary/    - Start/stop scripts"
    echo "  loki/log-scrapers/      - Fluent Bit, Vector, Alloy configs"
    echo ""
    echo "Usage:"
    echo "  cd loki/install/binary"
    echo "  ./start-loki.sh"
