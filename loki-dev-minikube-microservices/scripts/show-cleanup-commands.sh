...
# Check for help flag
if [[ "$1" == "--help" ]] || [[ "$1" == "-h" ]]; then
    echo "🧹 Cleanup Commands Reference Script"
    echo ""
    echo "📋 DESCRIPTION:"
    ...
    echo "  • Display organized cleanup commands by category"
    echo "  • Provide selective deletion options"
    echo "  • Show verification commands"
    echo "  • Safe reference-only operation"
    echo ""
    echo "🚀 USAGE:"
    ...
    echo "  • Loki Components        • Supporting Services"
    echo "  • Storage Resources       • Configuration Maps"
    echo "  • Secrets & RBAC         • Namespace Operations"
    echo ""
    echo "⚠️  SAFETY FEATURES:"
    echo "  • Read-only operation (displays commands only)"
    echo "  • No automatic execution"
    echo "  • User controls what gets deleted"
    echo "  • Organized by resource type"
    echo ""
    echo "🎯 USE CASES:"
    echo "  • Selective resource cleanup"
    echo "  • Troubleshooting specific components"
    echo "  • Learning kubectl delete syntax"
    echo "  • Partial environment cleanup"
    echo ""
    echo "📋 ALTERNATIVE:"
